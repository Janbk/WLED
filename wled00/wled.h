...
struct PSRAM_Allocator
{
  void *allocate(size_t size)
  {
    if (psramSafe && psramFound())
      return ps_malloc(size); // use PSRAM if it exists
    else
  ...
  {
    if (psramSafe && psramFound())
      return ps_realloc(ptr, new_size); // use PSRAM if it exists
    else
      return realloc(ptr, new_size); // fallback
  }
...
#define WLED_CODENAME "Demo Timer"
...
public:
  ...
  Receive(bool b, bool c, bool e, bool sO, bool sB, bool p)
      : Brightness(b), Color(c), Effects(e), SegmentOptions(sO), SegmentBounds(sB), Palette(p) {};
